<template>
    <div class="addEdit-block">
        <el-form
            ...
            <el-row>
                <el-col :span="12"  v-if="sessionTable !='yimiao'">
                    <el-form-item class="select" v-if="type!='info'"  label="疫苗信息" prop="yimiaoId">
                        <el-select v-model="ruleForm.yimiaoId" :disabled="ro.yimiaoId" filterable placeholder="请选择疫苗信息" @change="yimiaoChange">
                            <el-option
                                    v-for="(item,index) in yimiaoOptions"
                                    v-bind:key="item.id"
                                    :label="item.yimiaoName"
                                    :value="item.id">
                            </el-option>
                        </el-select>
                    </el-form-item>
                ...
                <el-col :span="12"  v-if="sessionTable !='yimiao' ">
                    <el-form-item class="input" v-if="type!='info'"  label="疫苗名称" prop="yimiaoName">
                        <el-input v-model="yimiaoForm.yimiaoName"
                                  placeholder="疫苗名称" clearable readonly></el-input>
                    </el-form-item>
                    <div v-else>
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="评价内容" prop="yimiaoCommentbackText">
                        <el-input type="textarea"  :readonly="ro.yimiaoCommentbackText" v-model="ruleForm.yimiaoCommentbackText" placeholder="评价内容"></el-input>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.yimiaoCommentbackText" label="评价内容" prop="yimiaoCommentbackText">
                            <span v-html="ruleForm.yimiaoCommentbackText"></span>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            if (this.role != "管理员"){
                this.ro.yimiaoId = true;//疫苗
                this.ro.yonghuId = true;//用户
                this.ro.yimiaoCommentbackText = true;//评价内容
            }
         ...
         this.$http({
             url: `yimiao/page?page=1&limit=100`,
             method: "get"
         }).then(({ data }) => {
             if (data && data.code === 0) {
                this.yimiaoOptions = data.data.list;
            }
         });
         this.$http({
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `yimiaoCommentback/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        _this.ruleForm = data.data;
                        _this.yimiaoChange(data.data.yimiaoId)
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`yimiaoCommentback/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.yimiaoCommentbackCrossAddOrUpdateFlag = false;
                                        this.parent.search();
