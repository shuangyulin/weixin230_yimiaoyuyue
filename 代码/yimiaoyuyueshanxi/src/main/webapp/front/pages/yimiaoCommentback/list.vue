<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.yimiaoCommentbackText" type="text" placeholder="评价内容"></input>
		</view>
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
		<view class="wrapper" style="margin-left: 25upx;margin-top: 10upx;width: 800rpx;">
			...
			<view class="address-box" style="margin-bottom: 10rpx;width: 600rpx;">
        ...
        <view style="float: right;margin-right: 25upx;">
            <text v-if="isAuth('yimiaoCommentback','删除')" style="margin: 0 50upx 0 30rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	<button
            ...
            v-if="isAuth('yimiaoCommentback','新增')" class="add-btn" @click="onAddTap()">新增</button>
