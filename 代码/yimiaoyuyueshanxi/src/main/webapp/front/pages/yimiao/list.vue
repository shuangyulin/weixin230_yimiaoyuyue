<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.yimiaoName" type="text" placeholder="疫苗名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in yimiaoTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.yimiaoName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.yimiaoPhoto?product.yimiaoPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.yimiaoNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
            <text class="">￥{{product.yimiaoNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            ...
            v-if="isAuth('yimiao','新增')" class="add-btn" @click="onAddTap()">新增</button>
