				...
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" style="color: red;textAlign:left">
						<text style='font-size:20rpx'>¥</text><text
							style="font-weight:bold;font-size:50rpx">{{detail.yimiaoNewMoney}}</text><text
							style="font-size: 20rpx;color: #8799A3;margin-left: 10rpx;">
							<text v-if="detail.yimiaoOldMoney!=null">原价￥<text
									style="text-decoration:line-through">{{detail.yimiaoOldMoney}}</text></text>
						</text>
					</view>
		...
		<view class="bottom-content cu-bar bg-white tabbar border shop">
			...
			<view style="text-align: right;">
				...
				<button @tap="onSubmit" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur"
					:style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3],borderColor:btnColor[3]}">立即预定</button>
			</view>
		</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('yimiao', this.id);
			...
			// 收藏
			async getyimiaoCollection() {
				let params = {
					page: 1,
					limit: 1,
					yimiaoId: this.id,
					yonghuId: this.user.id,
					yimiaoCollectionTypes: '1'
				}
				let res = await this.$api.list(`yimiaoCollection`, params);
			...
			async shoucang() {
				...
				let res = await _this.$api.list(`yimiaoCollection`, params);
				if (res.data.list.length == 1) {
					let yimiaoCollectionId = res.data.list[0].id;
					uni.showModal({
						title: '提示',
						content: '是否取消',
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.del('yimiaoCollection', JSON.stringify([
									yimiaoCollectionId]));
								_this.$utils.msg('取消成功');
								_this.getyimiaoCollection();
							}
						}
					});
					return;
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('yimiaoCollection', {
								yonghuId: _this.user.id,
								yimiaoId: _this.detail.id,
								yimiaoCollectionTypes: 1
							});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../yimiaoCommentback/add-or-update?yimiaoId=${this.id}`)
			},
		}
	}
</script>
